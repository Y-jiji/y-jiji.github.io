...
fn main() {
    ...
    let mut json_index = HashMap::<String, HashMap<serde_yaml::Value, Vec<String>>>::new();
                ...
                for (key, value) in mapping.into_iter() {
                    ...
                    let mut for_each_entry = |key: &String, value: serde_yaml::Value, file: &String| {
                        json_index.entry(key.to_string())
                            .and_modify(|e| {e.entry(value.clone()).and_modify(|e| e.push(file.to_string())).or_insert(vec![file.to_string()]); })
                            .or_insert(HashMap::from([(value, vec![file.to_string()])]));
                    };
